...
CHENTRY=$(echo -e "$TOGGLE\nmanual\n$LIST" | uniq -u | fuzzel $2 -d -l $LINENUM --prompt "Wi-Fi SSID: ")
#echo "$CHENTRY"
...
if [ "$CHENTRY" = "manual" ] ; then
	...
	MSSID=$(echo "enter the SSID of the network (SSID,password)" | fuzzel $2 -d --prompt "Manual Entry: ")
	...
	if [[ $(echo "$KNOWNCON" | grep "$CHSSID") = "$CHSSID" ]]; then
		...
		if [[ "$CHENTRY" =~ "WPA2" ]] || [[ "$CHENTRY" =~ "WEP" ]]; then
			WIFIPASS=$(echo "if connection is stored, hit enter" | fuzzel $2 --password -d --prompt "password")
		fi
